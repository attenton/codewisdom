...
export function Neo4jD3 (_selector, _options) {
  ...
  function appendRelationshipToGraph () {
    let relationship, text, outline, overlay
    relationship = appendRelationship()
...
//            color = options.colors[Math.min(numClasses, options.colors.length - 1)];
    ...
    if (options.infoPanel) {
      if (options.infoSelector) {
        ...
        info.attr('class', 'neo4jd3-info')
          .html('')
      } else { info = appendInfoPanel(container) }
    }
  ...
  function tickRelationshipsOutlines () {
    relationship.each(function (relationship) {
      ...
      outline.attr('d', function (d) {
        let center, angle, textBoundingBox, textPadding, u, textMargin, n, rotatedPointA1, rotatedPointB1,
          rotatedPointC1, rotatedPointD1, rotatedPointA2, rotatedPointB2, rotatedPointC2, rotatedPointD2,
          rotatedPointE2, rotatedPointF2, rotatedPointG2
        center = {x: 0, y: 0}
        ...
        // var dy1 = rotatedPointB2.y - rotatedPointA1.y
        // var dr1 = Math.sqrt(dx1 * dx1 + dy1 * dy1) * (d.linknum + 1.2) / (1.5 * 1.2)
        // var dx2 = rotatedPointD1.x - rotatedPointF2.x
        // var dy2 = rotatedPointD1.y - rotatedPointF2.y
        // var dr2 = (Math.sqrt(dx2 * dx2 + dy2 * dy2) * (d.linknum + 1.2) / (1.5 * 1.2))
        // console.log(dr1+" "+dr2)
        // console.log(dr1+"  "+dr2)
  ...
  function tickRelationshipsOverlays () {
    relationshipOverlay.attr('d', function (d) {
      let center, angle, n1, n, rotatedPointA, rotatedPointB, rotatedPointC, rotatedPointD
      center = {x: 0, y: 0}
      angle = rotation(d.source, d.target)
  ...
  function tickRelationshipsTexts () {
    relationshipText.attr('transform', function (d) {
      let angle, angle2, mirror, center, n, nWeight, point, rotatedPoint
      angle = rotation(d.source, d.target)
  ...
  function unitaryNormalVector (source, target, newLength) {
    let center = {x: 0, y: 0}
    let vector = unitaryVector(source, target, newLength)
  ...
  function updateNodes (n) {
    ...
    for (let newNode of n) {
      let exist = false
      for (let existNode of nodes) {
        if (newNode.id === existNode.id) {
          exist = true
        }
      }
      if (exist === false) {
  ...
  function removeNodeFromGraph () {
    let removeData = node.exit().remove()
  }

  function removeRelationFromGraph () {
    let removeData = relationship.exit().remove()
  }
  ...
  function updateRelationships (r) {
    ...
    for (let newRelation of r) {
      let exist = false
      for (let existRelation of relationships) {
        if (newRelation.id === existRelation.id) {
          exist = true
        }
      }
      if (exist === false) {
  ...
  function zoomFit (transitionDuration) {
    let bounds, parent, fullWidth, fullHeight, width, height, midX, midY
